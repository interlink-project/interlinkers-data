...
const InterlinkerBrowseFilter = ({ loading, filters, onFiltersChange, language }) => {
  ...
  return (<>
    ...
    <Box sx={{ mt: 1 }}>
      ...
      {filters.problemprofiles && filters.problemprofiles.map(pp => <Chip key={`active-filter-${pp}`} sx={{ mr: 1, mt: 1 }} label={`${t("Problem profile")}: ${pp}`} onDelete={() => changeFilter("problemprofiles", filters.problemprofiles.filter(problemprofile => problemprofile !== pp))} />)}
