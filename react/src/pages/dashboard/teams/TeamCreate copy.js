import {
  ...
  List,
  ListItem,
  ListItemAvatar, ListItemSecondaryAction, ListItemText, MobileStepper, TextField, Typography, useTheme
...
const TeamCreate = ({ language = getLanguage(), loading, setLoading, open, setOpen, onCreate }) => {
  ...
  const isDisabled = () => {
    ...
    if (activeStep === 1 && selectedUsers.length === 0) {
      return true
    }
  }
          ...
          {activeStep === 1 && <><List dense>
            {selectedUsers.map(user => {
              ...
              >
                ...
                <Button variant="link" sx={{color: "success.main"}}>
                Non administrator
                  </Button>
            ...
            {!selectedUsers.find(user => user.id === auth.user.id) && <Button fullWidth variant="text" color='primary' onClick={() => {
              setSelectedUsers([...selectedUsers, auth.user])
            }}
              sx={{ mt: 1 }}
            ><>{t("Add myself")}</></Button>}
